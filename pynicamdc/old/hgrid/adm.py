class Adm:
    ...
    def RGNMNG_setup(self, io_l, io_nml, fname_log, fname_in=None):
        ...
        if fname_in is None:
            ...
            if ( ADM_lall > RGNMNG_llim ):
                write(*,*) 'xxx limit exceed! local region: ', ADM_lall, RGNMNG_llim
                call prc.PRC_MPIstop
   
            if ADM_lall > RGNMNG_llim:
                if io_l:
                    with open(fname_log, 'a') as log_file: 
                        print('xxx limit exceed! local region:', ADM_lall, RGNMNG_llim, file=log_file)

                self.prc.PRC_MPIstop(io_l, fname_log)  
...
##endif
        ...
        if 'admparam' not in cnfs:
            ...
            self.ADM_lall = self.ADM_rgn_nmax // PRC_nprocs
            ...
            GLOBAL_extension_rgn = self.np.empty(ADM_lall, dtype=str)
            GLOBAL_extension_rgn[:] = ''
            # Allocate and initialize ADM_have_sgp with False
            ADM_have_sgp = self.np.full(ADM_lall, False, dtype=bool)
